...
//IMPORTANT: THIS CODE WORKS INDEED
...
char incoming_response[MAX_HTTP_REQUEST_LEN] = {0};


static void extract_incoming_response(void *payload, void *data_end, char *http_request) {
    ...
    for (int i = 0; i < remaining_space && payload + i < data_end; i++) {
        incoming_response[i] = ((char *)payload)[i];
        cursor++;
        remaining_space--;
    }

    // Null-terminate the string
    incoming_response[MAX_HTTP_REQUEST_LEN - remaining_space] = '\0';
}
...
int is_packet_http(struct xdp_md *ctx)
{
    ...
    extract_http_request(payload, data_end, incoming_response);

    if(incoming_response[0] == 'H' &&
       incoming_response[1] == 'T' &&
       incoming_response[2] == 'T' &&
       incoming_response[3] == 'P')
    {
        bpf_printk("%s", incoming_response);
        return 1;
    }
...
SEC("xdp")
int http_sniffer(struct xdp_md *ctx) {
